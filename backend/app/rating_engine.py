...
FUNDAMENTAL_RATIO = 0.20  # 基本面评分占比
AI_RATIO = 0.50     # AI评分占比

# 无基本面数据时的降级比例
QUANT_RATIO_NO_FUND = 0.38
AI_RATIO_NO_FUND = 0.62
...
async def rate_stock(df: pd.DataFrame, name: str = "", code: str = "", market: str = "") -> Optional[Dict]:
    ...
    if ai_result:
        ...
        if has_fund:
            # AI不可用: 量化60% + 基本面40%
            total = round(quant_total * 0.60 + fundamental_score * 0.40, 2)
        else:
