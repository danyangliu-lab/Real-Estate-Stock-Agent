...
5. 行业周期判断：当前处于房地产周期的哪个阶段，融资环境变化
...
你还需要：
1. 独立评估该股票当前的情绪倾向（sentiment_score，0-100，50为中性）
2. 评估各量化维度在当前市场环境下的参考价值（weight_hints，0-1）
   - 例如：在震荡整理期，波动率和成交量信号更可靠（给高值如0.7-0.8）
   - 在趋势明确期，趋势/动量指标更可靠（给高值如0.7-0.8）
...
def _build_ai_prompt(name: str, code: str, market: str, df: pd.DataFrame, quant_scores: Dict,
    ...
    prompt = f"""请对以下中国房地产相关股票进行深度分析，结合行情数据和财务指标，给出你独立的AI评分。
...
{fund_section}{announcement_section}【行情数据摘要】
...
1. **公司基本面（最关键）**：基于财务数据和你对该公司的了解，评估其经营质量、债务健康度（三道红线）、销售回款能力、土储质量。

2. **技术面与资金面**：分析中长期均线方向和支撑/压力位，成交量变化反映的机构资金态度。

3. **行业周期**：综合判断当前房地产行业周期阶段，融资环境对该公司的影响。

4. **风险评估**：债务违约风险、项目交付风险、系统性行业风险。

5. **AI增强因子建议**：基于你的判断，评估当前各量化指标的参考价值。例如：在震荡市，波动率和成交量指标更重要（给高权重）；在趋势明确期，趋势/动量指标更可靠。

请注意：你的评分应综合考虑基本面（40%）+ 技术面（35%）+ 风险（25%）。
同时，请独立评估该股票当前的情绪倾向（sentiment_score），以及各量化维度当前的参考价值（weight_hints）。

请输出以下JSON格式（不要输出其他内容）:
{{
  "ai_score": <0-100的整数，你独立给出的AI综合评分>,
  "sentiment_score": <0-100的整数，你对该股票当前情绪的独立评分，50为中性，>70偏积极，<30偏消极>,
  "weight_hints": {{
  ...
  "analysis": "<250字以内的专业分析，必须包含：1.公司基本面评价 2.技术面与资金面 3.财务/债务风险判断 4.综合操作建议>"
}}"""
    return prompt
...
def _fuse_tri_model_results(
    ...
    if minimax_result:
        ...
        weights.append(MINIMAX_WEIGHT)
        labels.append(f"MiniMax({minimax_result['ai_score']}分)")
    if glm_result:
...
async def get_ai_rating(name: str, code: str, market: str, df: pd.DataFrame,
    ...
    prompt = _build_ai_prompt(name, code, market, df, quant_scores, fundamentals, announcements)
    ...
    if MINIMAX_ENABLED:
        tasks.append(chat_minimax(prompt, system=AI_SYSTEM_PROMPT, temperature=0.3, enable_search=True))
...
async def rate_stock(df: pd.DataFrame, name: str = "", code: str = "", market: str = "") -> Optional[Dict]:
    ...
    # 2. 计算基础情绪因子（关键词+衰减模型，仅使用公告数据）
    base_sentiment = calc_sentiment_score("", announcements)
...
请重点关注：
1. 该公司是否收入增长稳健、三条红线达标、减值计提充分
2. PB是否<1，估值是否具有安全边际
3. 当前宏观利率环境和融资环境对房地产的影响
4. 技术面和资金面的趋势信号
...
{{"score": 数字, "analysis": "你的详细分析"}}"""
        ...
        if MINIMAX_ENABLED:
            tasks.append(chat_minimax(user_msg, system=SOOCHOW_AI_PROMPT, temperature=0.3, enable_search=True))
            task_labels.append("MiniMax")
        if GLM_ENABLED:
...
async def rate_stock_soochow(df: pd.DataFrame, name: str = "", code: str = "",
    ...
    # 情绪因子（仅使用公告数据）
    sentiment = calc_sentiment_score("", ann_text)
